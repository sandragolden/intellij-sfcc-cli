...
const decrypt = (source, target, secretKey, host, username) => new Promise((resolve, reject) => {
    ...
    if (!fs.existsSync(source)) {
        reject(`The source file "${source}" does not exist. Aborting...`);
        return;
    }
    ...
    if (host && decryptedJson) {
        if (!username) {
            reject('Please provide username of access key! Aborting...');
            return;
        }
        ...
        if (!account || !account.accessKeys) {
            reject(`Could not find access keys for account matching username of "${username}"! Aborting...`);
            return;
        }
