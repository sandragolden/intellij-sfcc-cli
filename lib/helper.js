...
const CREDENTIALS_FILE = process.env.SFCC_INTELLIJ_CREDENTIALS_FILE || path.resolve(__dirname, '../intellij-sfcc-credentials.creds');
const JSON_FILE = process.env.SFCC_INTELLIJ_JSON_FILE || path.resolve(__dirname, '../intellij-sfcc-credentials.json');
...
const decrypt = (source, target, secretKey, host, username) => new Promise((resolve, reject) => {
    ...
    if (!secretKey) {
        reject(new Error('Please provide secret key to decrypt the file! Aborting...'));
        return;
    }
